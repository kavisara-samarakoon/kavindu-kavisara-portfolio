...
// Features: big animated name, role, tagline, and two CTA buttons
// Uses GSAP for entrance animation (fires once on mount)
"use client";
...
export default function Hero() {
  // Refs to grab DOM elements for GSAP to target
  ...
  useEffect(() => {
    // Create a GSAP timeline for the staggered entrance animation
    const ctx = gsap.context(() => {
      const tl = gsap.timeline({ defaults: { ease: "power3.out" } });

      // Animate the name in — big slide up
      tl.fromTo(
        nameRef.current,
        { y: 80, opacity: 0 },
        { y: 0, opacity: 1, duration: 1 }
      )
        // Role badge slides in
        .fromTo(
          roleRef.current,
          { y: 30, opacity: 0 },
    ...
    >
      ...
      <div className="container-custom flex flex-col gap-8">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-8">
          ...
          <p
            ref={descRef}
            className="opacity-0 font-dm text-muted text-base md:text-lg max-w-lg leading-relaxed"
          >
            I design and build clean, modern web experiences while growing my
            skills in networking, cybersecurity, and full-stack development.
      ...
      {/* Inline style for scroll line animation */}
      <style jsx>{`
